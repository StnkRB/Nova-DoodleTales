# 🎨 Bringing Art to Life: How We Built DoodleTales with Amazon Nova
...
## The Engine: Amazon Bedrock & Nova

Building a real-time, multimodal experience requires a stack that is both powerful and incredibly flexible. We chose **Amazon Bedrock** and the **Amazon Nova** family of models to power every layer of DoodleTales.

### 1. Seeing the World with Amazon Nova Pro
The first step is understanding the art. When a child uploads a photo of their drawing, we use **Amazon Nova Pro** to perform high-precision character segmentation. 
...
### 2. Talking with Amazon Nova Pro (Multimodal)
The heart of DoodleTales is the **Storytelling Scout**. We used **Amazon Nova Pro's** multimodal capabilities to create a conversational experience.
...
### 3. Expanding the Story with Amazon Nova Canvas
...
- **Style Reference**: We use **Amazon Nova Canvas's** image-to-image capabilities with the original drawing as a **Style Reference**.
...
- **Global Reach**: Deploying via Cloud Run (connected to Bedrock) allows us to serve users globally with low latency.
